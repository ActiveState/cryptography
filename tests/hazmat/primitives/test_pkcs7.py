...
class TestPKCS7Loading(object):
    ...
    def test_load_pkcs7_der(self):
        certs = load_vectors_from_file(os.path.join('pkcs7', 'amazon-roots.p7b'), lambda derfile: pkcs7.load_der_pkcs7_certificates(derfile.read()), mode='rb')
        assert len(certs) == 2
...
def _pkcs7_verify(encoding, sig, msg, certs, options, backend):
    ...
    if msg is None:
        ...
        res = backend._lib.PKCS7_verify(p7, backend._ffi.NULL, store, msg_bio.bio, backend._ffi.NULL, flags)
    backend.openssl_assert(res == 1)
...
class TestPKCS7Builder(object):
    ...
    def test_sign_invalid_options(self):
        ...
        with pytest.raises(ValueError):  # type: ignore[list-item]
    ...
    @pytest.mark.parametrize(('hash_alg', 'expected_value'), [(hashes.SHA1(), b'\x06\x05+\x0e\x03\x02\x1a'), (hashes.SHA256(), b'\x06\t`\x86H\x01e\x03\x04\x02\x01'), (hashes.SHA384(), b'\x06\t`\x86H\x01e\x03\x04\x02\x02'), (hashes.SHA512(), b'\x06\t`\x86H\x01e\x03\x04\x02\x03')])
    ...
    @pytest.mark.parametrize(('hash_alg', 'expected_value'), [(hashes.SHA1(), b'sha1'), (hashes.SHA256(), b'sha-256'), (hashes.SHA384(), b'sha-384'), (hashes.SHA512(), b'sha-512')])
