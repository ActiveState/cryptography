...
class TestTOTP(object):
    ...
    def test_get_provisioning_uri(self, backend):
        ...
        assert (
            totp.get_provisioning_uri("Alice Smith", None)
            == "otpauth://totp/Alice%20Smith?digits=6&secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ&algorithm=SHA1&period=30"
        )
        assert (
            totp.get_provisioning_uri("Alice Smith", "World")
            == "otpauth://totp/World:Alice%20Smith?digits=6&secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ&algorithm=SHA1&issuer=World&period=30"
        )
