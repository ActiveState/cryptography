...
class TestHOTP(object):
    ...
    def test_get_provisioning_uri(self, backend):
        ...
        hotp = HOTP(secret, 6, SHA1(), backend)
        assert (
            hotp.get_provisioning_uri("Alice Smith", 1, None)
            == "otpauth://hotp/Alice%20Smith?digits=6&secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ&algorithm=SHA1&counter=1"
        )
        assert (
            hotp.get_provisioning_uri("Alice Smith", 1, "Foo")
            == "otpauth://hotp/Foo:Alice%20Smith?digits=6&secret=GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ&algorithm=SHA1&issuer=Foo&counter=1"
        )
