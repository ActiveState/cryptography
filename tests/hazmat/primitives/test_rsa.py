...
from ...doubles import DummyAsymmetricPadding, DummyHashAlgorithm, DummyKeySerializationEncryption
...
from .fixtures_rsa import RSA_KEY_512, RSA_KEY_522, RSA_KEY_599, RSA_KEY_745, RSA_KEY_1024, RSA_KEY_1025, RSA_KEY_1026, RSA_KEY_1027, RSA_KEY_1028, RSA_KEY_1029, RSA_KEY_1030, RSA_KEY_1031, RSA_KEY_1536, RSA_KEY_2048, RSA_KEY_2048_ALT, RSA_KEY_CORRUPTED
from .utils import _check_rsa_private_numbers, generate_rsa_verification_test, skip_fips_traditional_openssl
...
def _build_oaep_sha2_vectors():
    ...
    for mgf1alg, oaepalg in itertools.product(hashalgs, hashalgs):
        if mgf1alg.name == 'sha1' and oaepalg.name == 'sha1':
            ...
            continue
        examples = _flatten_pkcs1_examples(load_vectors_from_file(os.path.join(base_path, 'oaep-{}-{}.txt'.format(mgf1alg.name, oaepalg.name)), load_pkcs1_vectors))
...
def _skip_pss_hash_algorithm_unsupported(backend, hash_alg):
    if not backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hash_alg), salt_length=padding.PSS.MAX_LENGTH)):
...
def test_modular_inverse():
    p = int('', 16)
    q = int('', 16)
    assert rsa._modinv(q, p) == int('', 16)

class TestRSA(object):

    @pytest.mark.parametrize(('public_exponent', 'key_size'), itertools.product((3, 65537), (1024, 1025, 1026, 1027, 1028, 1029, 1030, 1031, 1536, 2048)))
    ...
    def test_generate_bad_public_exponent(self, backend):
        ...
        with pytest.raises(ValueError):
            rsa.generate_private_key(public_exponent=65535, key_size=2048, backend=backend)

    def test_cant_generate_insecure_tiny_key(self, backend):
        with pytest.raises(ValueError):
            rsa.generate_private_key(public_exponent=65537, key_size=511, backend=backend)
        with pytest.raises(ValueError):
            rsa.generate_private_key(public_exponent=65537, key_size=256, backend=backend)

    @pytest.mark.parametrize('pkcs1_example', load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'pkcs-1v2-1d2-vec', 'pss-vect.txt'), load_pkcs1_vectors))
    def test_load_pss_vect_example_keys(self, pkcs1_example):
        secret, public = pkcs1_example
        private_num = rsa.RSAPrivateNumbers(p=secret['p'], q=secret['q'], d=secret['private_exponent'], dmp1=secret['dmp1'], dmq1=secret['dmq1'], iqmp=secret['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=secret['public_exponent'], n=secret['modulus']))
        _check_rsa_private_numbers(private_num)
        public_num = rsa.RSAPublicNumbers(e=public['public_exponent'], n=public['modulus'])
        assert public_num
    ...
    @pytest.mark.parametrize('vector', load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'oaep-label.txt'), load_nist_vectors))
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=b'label')), skip_message='Does not support RSA OAEP labels')
    def test_oaep_label_decrypt(self, vector, backend):
        private_key = serialization.load_der_private_key(binascii.unhexlify(vector['key']), None, backend)
        assert isinstance(private_key, rsa.RSAPrivateKey)
        assert vector['oaepdigest'] == b'SHA512'
        decrypted = private_key.decrypt(binascii.unhexlify(vector['input']), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA512()), algorithm=hashes.SHA512(), label=binascii.unhexlify(vector['oaeplabel'])))
        assert vector['output'][1:-1] == decrypted

    @pytest.mark.parametrize(('msg', 'label'), [(b'amazing encrypted msg', b'some label'), (b'amazing encrypted msg', b'')])
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=b'label')), skip_message='Does not support RSA OAEP labels')
    def test_oaep_label_roundtrip(self, msg, label, backend):
        private_key = RSA_KEY_2048.private_key(backend)
        ct = private_key.public_key().encrypt(msg, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=label))
        pt = private_key.decrypt(ct, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=label))
        assert pt == msg

    @pytest.mark.parametrize(('enclabel', 'declabel'), [(b'label1', b'label2'), (b'label3', b''), (b'', b'label4')])
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=b'label')), skip_message='Does not support RSA OAEP labels')
    def test_oaep_wrong_label(self, enclabel, declabel, backend):
        private_key = RSA_KEY_2048.private_key(backend)
        msg = b'test'
        ct = private_key.public_key().encrypt(msg, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=enclabel))
        with pytest.raises(ValueError):
            private_key.decrypt(ct, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=declabel))

    @pytest.mark.supported(only_if=lambda backend: not backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=b'label')), skip_message='Requires backend without RSA OAEP label support')
    def test_unsupported_oaep_label_decrypt(self, backend):
        private_key = RSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.decrypt(b'0' * 64, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=b'label'))
...
class TestRSASignature(object):
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PKCS1v15()), skip_message='Does not support PKCS1v1.5.')
    ...
    def test_pkcs1v15_signing(self, backend, disable_rsa_checks, subtests):
        ...
        for private, public, example in vectors:
            with subtests.test():
                private_key = rsa.RSAPrivateNumbers(p=private['p'], q=private['q'], d=private['private_exponent'], dmp1=private['dmp1'], dmq1=private['dmq1'], iqmp=private['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=private['public_exponent'], n=private['modulus'])).private_key(backend)
                signature = private_key.sign(binascii.unhexlify(example['message']), padding.PKCS1v15(), hashes.SHA1())
                assert binascii.hexlify(signature) == example['signature']

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH)), skip_message='Does not support PSS.')
    @pytest.mark.parametrize('pkcs1_example', _flatten_pkcs1_examples(load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'pkcs-1v2-1d2-vec', 'pss-vect.txt'), load_pkcs1_vectors)))
    @pytest.mark.skip_fips(reason='SHA1 signing not supported in FIPS mode.')
    def test_pss_signing(self, pkcs1_example, backend):
        ...
        signature = private_key.sign(binascii.unhexlify(example['message']), padding.PSS(mgf=padding.MGF1(algorithm=hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA1())
        ...
        public_key.verify(signature, binascii.unhexlify(example['message']), padding.PSS(mgf=padding.MGF1(algorithm=hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA1())

    @pytest.mark.parametrize('hash_alg', [hashes.SHA224(), hashes.SHA256(), hashes.SHA384(), hashes.SHA512()])
    def test_pss_signing_sha2(self, hash_alg, backend):
        _skip_pss_hash_algorithm_unsupported(backend, hash_alg)
        private_key = RSA_KEY_2048.private_key(backend)
        public_key = private_key.public_key()
        pss = padding.PSS(mgf=padding.MGF1(hash_alg), salt_length=padding.PSS.MAX_LENGTH)
        msg = b'testing signature'
        signature = private_key.sign(msg, pss, hash_alg)
        public_key.verify(signature, msg, pss, hash_alg)

    @pytest.mark.supported(only_if=lambda backend: backend.hash_supported(hashes.SHA512()) and backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH)), skip_message='Does not support SHA512.')
    ...
    def test_pss_minimum_key_size_for_digest(self, backend):
        ...
        private_key.sign(b'no failure', padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA512())
    ...
    def test_pss_signing_digest_too_large_for_key_size(self, backend):
        private_key = RSA_KEY_512.private_key(backend)
        with pytest.raises(ValueError):
            private_key.sign(b'msg', padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA512())

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=padding.PSS.MAX_LENGTH)), skip_message='Does not support PSS.')
    def test_pss_signing_salt_length_too_long(self, backend):
        private_key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(ValueError):
            private_key.sign(b'failure coming', padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=1000000), hashes.SHA256())
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=0)), skip_message='Does not support PSS.')
    def test_unsupported_pss_mgf(self, backend):
        private_key = RSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_MGF):  # type: ignore[arg-type]
            private_key.sign(b'msg', padding.PSS(mgf=DummyMGF(), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA1())
    ...
    def test_prehashed_unsupported_in_signer_ctx(self, backend):
        private_key = RSA_KEY_512.private_key(backend)
        with pytest.raises(TypeError), pytest.warns(CryptographyDeprecationWarning):  # type: ignore[arg-type]
            private_key.signer(padding.PKCS1v15(), asym_utils.Prehashed(hashes.SHA1()))

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PKCS1v15()), skip_message='Does not support PKCS1v1.5.')
    def test_prehashed_unsupported_in_verifier_ctx(self, backend):
        public_key = RSA_KEY_512.private_key(backend).public_key()
        with pytest.raises(TypeError), pytest.warns(CryptographyDeprecationWarning):  # type: ignore[arg-type]
            public_key.verifier(b'0' * 64, padding.PKCS1v15(), asym_utils.Prehashed(hashes.SHA1()))
    ...
    def test_prehashed_unsupported_in_signature_recover(self, backend):
        ...
        prehashed_alg = asym_utils.Prehashed(hashes.SHA256())
        with pytest.raises(TypeError):  # type: ignore[arg-type]
            public_key.recover_data_from_signature(signature, padding.PKCS1v15(), prehashed_alg)

    def test_corrupted_private_key(self, backend):
        with pytest.raises(ValueError):
            serialization.load_pem_private_key(RSA_KEY_CORRUPTED, password=None, backend=backend)
...
class TestRSAVerification(object):
    ...
    def test_invalid_signature_sequence_removed(self, backend):
        ...
        """
        key_der = binascii.unhexlify(b'')
        sig = binascii.unhexlify(b'')
        public_key = serialization.load_der_public_key(key_der, backend)
        assert isinstance(public_key, rsa.RSAPublicKey)
        with pytest.raises(InvalidSignature):
            public_key.verify(sig, binascii.unhexlify(b'313233343030'), padding.PKCS1v15(), hashes.SHA256())
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=20)), skip_message='Does not support PSS.')
    @pytest.mark.parametrize('pkcs1_example', _flatten_pkcs1_examples(load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'pkcs-1v2-1d2-vec', 'pss-vect.txt'), load_pkcs1_vectors)))
    ...
    def test_invalid_pss_signature_wrong_data(self, backend):
        public_key = rsa.RSAPublicNumbers(n=int(b'dffc2137d5e810cde9e4b4612f5796447218bab913b3fa98bdf7982e4fa6ec4d6653ef2b29fb1642b095befcbea6decc178fb4bed243d3c3592c68546af2d3f3', 16), e=65537).public_key(backend)
    ...
    def test_invalid_pss_signature_wrong_key(self, backend):
        signature = binascii.unhexlify(b'3a1880165014ba6eb53cc1449d13e5132ebcc0cfd9ade6d7a2494a0503bd0826f8a46c431e0d7be0ca3e453f8b2b009e2733764da7927cc6dbe7a021437a242e')
        public_key = rsa.RSAPublicNumbers(n=int(b'', 16), e=65537).public_key(backend)
        with pytest.raises(InvalidSignature):
    ...
    def test_invalid_pss_signature_data_too_large_for_modulus(self, backend):
        ...
        signature = binascii.unhexlify(b'')
        # 1024 bit key
    ...
    def test_unsupported_pss_mgf(self, backend):
        ...
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_MGF):
            public_key.verify(b'sig', b'msg', padding.PSS(mgf=DummyMGF(), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA1())
    ...
    def test_pss_verify_digest_too_large_for_key_size(self, backend):
        private_key = RSA_KEY_512.private_key(backend)
        signature = binascii.unhexlify(b'8b9a3ae9fb3b64158f3476dd8d8a1f1425444e98940e0926378baa9944d219d8534c050ef6b19b1bdc6eb4da422e89161106a6f5b5cc16135b11eb6439b646bd')
        public_key = private_key.public_key()
        with pytest.raises(ValueError):
    ...
    def test_pss_verify_salt_length_too_long(self, backend):
        signature = binascii.unhexlify(b'8b9a3ae9fb3b64158f3476dd8d8a1f1425444e98940e0926378baa9944d219d8534c050ef6b19b1bdc6eb4da422e89161106a6f5b5cc16135b11eb6439b646bd')
        public_key = rsa.RSAPublicNumbers(n=int(b'd309e4612809437548b747d7f9eb9cd3340f54fe42bb3f84a36933b0839c11b0c8b7f67e11f7252370161e31159c49c784d4bc41c42a78ce0f0b40a3ca8ffb91', 16), e=65537).public_key(backend)
        with pytest.raises(InvalidSignature):
...
class TestRSADecryption(object):
    ...
    def test_decrypt_pkcs1v15_vectors(self, backend, disable_rsa_checks, subtests):
        vectors = _flatten_pkcs1_examples(load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'pkcs1v15crypt-vectors.txt'), load_pkcs1_vectors))
        for private, public, example in vectors:
            with subtests.test():
                skey = rsa.RSAPrivateNumbers(p=private['p'], q=private['q'], d=private['private_exponent'], dmp1=private['dmp1'], dmq1=private['dmq1'], iqmp=private['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=private['public_exponent'], n=private['modulus'])).private_key(backend)
    ...
    def test_decrypt_ciphertext_too_small(self, backend):
        private_key = RSA_KEY_2048.private_key(backend)
        ct = binascii.unhexlify(b'50b4c14136bd198c2f3c3ed243fce036e168d56517984a263cd66492b80804f169d210f2b9bdfb48b12f9ea05009c77da257cc600ccefe3a6283789d8ea0')
        with pytest.raises(ValueError):
            private_key.decrypt(ct, padding.PKCS1v15())

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None)), skip_message='Does not support OAEP.')
    @pytest.mark.parametrize('vector', _flatten_pkcs1_examples(load_vectors_from_file(os.path.join('asymmetric', 'RSA', 'pkcs-1v2-1d2-vec', 'oaep-vect.txt'), load_pkcs1_vectors)))
    def test_decrypt_oaep_vectors(self, vector, backend):
        private, public, example = vector
        skey = rsa.RSAPrivateNumbers(p=private['p'], q=private['q'], d=private['private_exponent'], dmp1=private['dmp1'], dmq1=private['dmq1'], iqmp=private['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=private['public_exponent'], n=private['modulus'])).private_key(backend)
        message = skey.decrypt(binascii.unhexlify(example['encryption']), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None))
        assert message == binascii.unhexlify(example['message'])

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA224()), algorithm=hashes.SHA224(), label=None)), skip_message='Does not support OAEP using SHA224 MGF1 and SHA224 hash.')
    def test_decrypt_oaep_sha2_vectors(self, backend, disable_rsa_checks, subtests):
        ...
        for private, public, example, mgf1_alg, hash_alg in vectors:
            with subtests.test():
                skey = rsa.RSAPrivateNumbers(p=private['p'], q=private['q'], d=private['private_exponent'], dmp1=private['dmp1'], dmq1=private['dmq1'], iqmp=private['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=private['public_exponent'], n=private['modulus'])).private_key(backend)
                message = skey.decrypt(binascii.unhexlify(example['encryption']), padding.OAEP(mgf=padding.MGF1(algorithm=mgf1_alg), algorithm=hash_alg, label=None))
                assert message == binascii.unhexlify(example['message'])
    ...
    def test_invalid_oaep_decryption(self, backend):
        ...
        # it.
        private_key = RSA_KEY_2048.private_key(backend)
        ciphertext = private_key.public_key().encrypt(b'secure data', padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None))
        private_key_alt = RSA_KEY_2048_ALT.private_key(backend)
        with pytest.raises(ValueError):
            private_key_alt.decrypt(ciphertext, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None))
    ...
    def test_invalid_oaep_decryption_data_to_large_for_modulus(self, backend):
        ...
        with pytest.raises(ValueError):
            key.decrypt(ciphertext, padding.OAEP(algorithm=hashes.SHA1(), mgf=padding.MGF1(hashes.SHA1()), label=None))

    def test_unsupported_oaep_mgf(self, backend):
        private_key = RSA_KEY_2048.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_MGF):  # type: ignore[arg-type]
            private_key.decrypt(b'0' * 256, padding.OAEP(mgf=DummyMGF(), algorithm=hashes.SHA1(), label=None))

class TestRSAEncryption(object):

    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None)), skip_message='Does not support OAEP.')
    @pytest.mark.parametrize(('key_data', 'pad'), itertools.product((RSA_KEY_1024, RSA_KEY_1025, RSA_KEY_1026, RSA_KEY_1027, RSA_KEY_1028, RSA_KEY_1029, RSA_KEY_1030, RSA_KEY_1031, RSA_KEY_1536, RSA_KEY_2048), [padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None)]))
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA512(), label=None)), skip_message='Does not support OAEP using SHA256 MGF1 and SHA512 hash.')
    @pytest.mark.parametrize(('mgf1hash', 'oaephash'), itertools.product([hashes.SHA1(), hashes.SHA224(), hashes.SHA256(), hashes.SHA384(), hashes.SHA512()], [hashes.SHA1(), hashes.SHA224(), hashes.SHA256(), hashes.SHA384(), hashes.SHA512()]))
    def test_rsa_encrypt_oaep_sha2(self, mgf1hash, oaephash, backend):
        pad = padding.OAEP(mgf=padding.MGF1(algorithm=mgf1hash), algorithm=oaephash, label=None)
        private_key = RSA_KEY_2048.private_key(backend)
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.rsa_padding_supported(padding.PKCS1v15()), skip_message='Does not support PKCS1v1.5.')
    @pytest.mark.parametrize(('key_data', 'pad'), itertools.product((RSA_KEY_1024, RSA_KEY_1025, RSA_KEY_1026, RSA_KEY_1027, RSA_KEY_1028, RSA_KEY_1029, RSA_KEY_1030, RSA_KEY_1031, RSA_KEY_1536, RSA_KEY_2048), [padding.PKCS1v15()]))
    ...
    @pytest.mark.parametrize(('key_data', 'pad'), itertools.product((RSA_KEY_1024, RSA_KEY_1025, RSA_KEY_1026, RSA_KEY_1027, RSA_KEY_1028, RSA_KEY_1029, RSA_KEY_1030, RSA_KEY_1031, RSA_KEY_1536, RSA_KEY_2048), (padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None), padding.PKCS1v15())))
    ...
    def test_unsupported_oaep_mgf(self, backend):
        ...
        public_key = private_key.public_key()
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_MGF):  # type: ignore[arg-type]
            public_key.encrypt(b'ciphertext', padding.OAEP(mgf=DummyMGF(), algorithm=hashes.SHA1(), label=None))
...
class TestRSANumbers(object):
    ...
    def test_rsa_private_numbers(self):
        public_numbers = rsa.RSAPublicNumbers(e=1, n=15)
        private_numbers = rsa.RSAPrivateNumbers(p=3, q=5, d=1, dmp1=1, dmq1=1, iqmp=2, public_numbers=public_numbers)
    ...
    def test_invalid_public_numbers_argument_values(self, e, n, backend):
        ...
        with pytest.raises(ValueError):
            rsa.RSAPublicNumbers(e=e, n=n).public_key(backend)  # modulus < 3
    # modulus != p * q
    # p > modulus
    # q > modulus
    # dmp1 > modulus
    ...
    # public_exponent < 3
    # public_exponent > modulus
    # public_exponent is not odd
    # dmp1 is not odd
    # dmq1 is not odd

    @pytest.mark.parametrize(('p', 'q', 'd', 'dmp1', 'dmq1', 'iqmp', 'e', 'n'), [(3, 11, 3, 1, 3, 2, 7, 2), (3, 11, 3, 1, 3, 2, 7, 35), (37, 11, 3, 1, 3, 2, 7, 33), (3, 37, 3, 1, 3, 2, 7, 33), (3, 11, 3, 35, 3, 2, 7, 33), (3, 11, 3, 1, 35, 2, 7, 33), (3, 11, 3, 1, 3, 35, 7, 33), (3, 11, 37, 1, 3, 2, 7, 33), (3, 11, 3, 1, 3, 2, 1, 33), (3, 11, 3, 1, 3, 35, 65537, 33), (3, 11, 3, 1, 3, 2, 6, 33), (3, 11, 3, 2, 3, 2, 7, 33), (3, 11, 3, 1, 4, 2, 7, 33)])
    def test_invalid_private_numbers_argument_values(self, p, q, d, dmp1, dmq1, iqmp, e, n, backend):
        ...
        with pytest.raises(ValueError):
            rsa.RSAPrivateNumbers(p=p, q=q, d=d, dmp1=dmp1, dmq1=dmq1, iqmp=iqmp, public_numbers=rsa.RSAPublicNumbers(e=e, n=n)).private_key(backend)
...
class TestRSAPrimeFactorRecovery(object):
    ...
    def test_recover_prime_factors(self, subtests):
        ...
        for vector in vectors:
            with subtests.test():
                private, public, example = vector
                p, q = rsa.rsa_recover_prime_factors(private['modulus'], private['public_exponent'], private['private_exponent'])
...
class TestRSAPrivateKeySerialization(object):
    ...
    @pytest.mark.parametrize(('fmt', 'password'), itertools.product([serialization.PrivateFormat.TraditionalOpenSSL, serialization.PrivateFormat.PKCS8], [b's', b'longerpassword', b'!*$&(@#$*&($T@%_somesymbols', b'\x01' * 1000]))
    ...
    @pytest.mark.parametrize(('fmt', 'password'), [[serialization.PrivateFormat.PKCS8, b's'], [serialization.PrivateFormat.PKCS8, b'longerpassword'], [serialization.PrivateFormat.PKCS8, b'!*$&(@#$*&($T@%_somesymbol'], [serialization.PrivateFormat.PKCS8, b'\x01' * 1000]])
    ...
    @pytest.mark.parametrize(('encoding', 'fmt', 'loader_func'), [[serialization.Encoding.PEM, serialization.PrivateFormat.TraditionalOpenSSL, serialization.load_pem_private_key], [serialization.Encoding.DER, serialization.PrivateFormat.TraditionalOpenSSL, serialization.load_der_private_key], [serialization.Encoding.PEM, serialization.PrivateFormat.PKCS8, serialization.load_pem_private_key], [serialization.Encoding.DER, serialization.PrivateFormat.PKCS8, serialization.load_der_private_key]])
    ...
    @pytest.mark.skip_fips(reason='Traditional OpenSSL key format is not supported in FIPS mode.')
    @pytest.mark.parametrize(('key_path', 'encoding', 'loader_func'), [[os.path.join('asymmetric', 'Traditional_OpenSSL_Serialization', 'testrsa.pem'), serialization.Encoding.PEM, serialization.load_pem_private_key], [os.path.join('asymmetric', 'DER_Serialization', 'testrsa.der'), serialization.Encoding.DER, serialization.load_der_private_key]])
    def test_private_bytes_traditional_openssl_unencrypted(self, backend, key_path, encoding, loader_func):
        key_bytes = load_vectors_from_file(key_path, lambda pemfile: pemfile.read(), mode='rb')
        key = loader_func(key_bytes, None, backend)
        serialized = key.private_bytes(encoding, serialization.PrivateFormat.TraditionalOpenSSL, serialization.NoEncryption())
        assert serialized == key_bytes

    def test_private_bytes_traditional_der_encrypted_invalid(self, backend):
        key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(ValueError):
            key.private_bytes(serialization.Encoding.DER, serialization.PrivateFormat.TraditionalOpenSSL, serialization.BestAvailableEncryption(b'password'))
    ...
    def test_private_bytes_invalid_encryption_algorithm(self, backend):
        key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(TypeError):  # type: ignore[arg-type]
            key.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.TraditionalOpenSSL, 'notanencalg')

    def test_private_bytes_unsupported_encryption_type(self, backend):
        key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(ValueError):
            key.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.TraditionalOpenSSL, DummyKeySerializationEncryption())
...
class TestRSAPEMPublicKeySerialization(object):
    ...
    def test_public_bytes_openssh(self, backend):
        key_bytes = load_vectors_from_file(os.path.join('asymmetric', 'public', 'PKCS1', 'rsa.pub.pem'), lambda pemfile: pemfile.read(), mode='rb')
        key = serialization.load_pem_public_key(key_bytes, backend)
        ssh_bytes = key.public_bytes(serialization.Encoding.OpenSSH, serialization.PublicFormat.OpenSSH)
        assert ssh_bytes == b'ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAgQC7JHoJfg6yNzLMOWet8Z49a4KD0dCspMAYvo2YAMB7/wdEycocujbhJ2n/seONi+5XqTqqFkM5VBl8rmkkFPZk/7x0xmdsTPECSWnHK+HhoaNDFPR3j8jQhVo1laxiqcEhAHegi5cwtFosuJAvSKAFKEvyD43si00DQnXWrYHAEQ=='
        with pytest.raises(ValueError):
            key.public_bytes(serialization.Encoding.PEM, serialization.PublicFormat.OpenSSH)
        with pytest.raises(ValueError):
            key.public_bytes(serialization.Encoding.DER, serialization.PublicFormat.OpenSSH)
        with pytest.raises(ValueError):
            key.public_bytes(serialization.Encoding.OpenSSH, serialization.PublicFormat.PKCS1)
        with pytest.raises(ValueError):
            key.public_bytes(serialization.Encoding.OpenSSH, serialization.PublicFormat.SubjectPublicKeyInfo)
    ...
    @pytest.mark.parametrize(('encoding', 'fmt'), [(serialization.Encoding.Raw, serialization.PublicFormat.SubjectPublicKeyInfo), (serialization.Encoding.Raw, serialization.PublicFormat.PKCS1)] + list(itertools.product([serialization.Encoding.Raw, serialization.Encoding.X962, serialization.Encoding.PEM, serialization.Encoding.DER], [serialization.PublicFormat.Raw, serialization.PublicFormat.UncompressedPoint, serialization.PublicFormat.CompressedPoint])))
