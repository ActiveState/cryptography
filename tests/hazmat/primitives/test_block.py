...
class TestCipher(object):
    ...
    def test_creates_encryptor(self, backend):
        cipher = Cipher(algorithms.AES(binascii.unhexlify(b'0' * 32)), modes.CBC(binascii.unhexlify(b'0' * 32)), backend)
...
class TestCipherContext(object):
    ...
    def test_unaligned_block_encryption(self, backend):
        cipher = Cipher(algorithms.AES(binascii.unhexlify(b'0' * 32)), modes.ECB(), backend)
        encryptor = cipher.encryptor()
...
class TestAEADCipherContext(object):
    test_aead_exceptions = generate_aead_exception_test(algorithms.AES, modes.GCM)
...
class TestModeValidation(object):
    ...
    def test_cbc(self, backend):
        with pytest.raises(ValueError):
            Cipher(algorithms.AES(b'\x00' * 16), modes.CBC(b'abc'), backend)

    def test_ofb(self, backend):
        with pytest.raises(ValueError):
            Cipher(algorithms.AES(b'\x00' * 16), modes.OFB(b'abc'), backend)

    def test_cfb(self, backend):
        with pytest.raises(ValueError):
            Cipher(algorithms.AES(b'\x00' * 16), modes.CFB(b'abc'), backend)

    def test_cfb8(self, backend):
        with pytest.raises(ValueError):
            Cipher(algorithms.AES(b'\x00' * 16), modes.CFB8(b'abc'), backend)

    def test_ctr(self, backend):
        with pytest.raises(ValueError):
            Cipher(algorithms.AES(b'\x00' * 16), modes.CTR(b'abc'), backend)
