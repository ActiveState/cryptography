...
def _load_all_params(path, file_names, param_loader):
    ...
    for file_name in file_names:
        all_params.extend(load_vectors_from_file(os.path.join(path, file_name), param_loader))
    return all_params
...
def aead_test(backend, cipher_factory, mode_factory, params):
    ...
    if params.get('fail') is True:
        cipher = Cipher(cipher_factory(binascii.unhexlify(params['key'])), mode_factory(binascii.unhexlify(params['iv']), binascii.unhexlify(params['tag']), len(binascii.unhexlify(params['tag']))), backend)
        decryptor = cipher.decryptor()
        ...
        cipher = Cipher(cipher_factory(binascii.unhexlify(params['key'])), mode_factory(binascii.unhexlify(params['iv']), binascii.unhexlify(params['tag']), min_tag_length=tag_len), backend)
        decryptor = cipher.decryptor()
        decryptor.authenticate_additional_data(binascii.unhexlify(aad))
        actual_plaintext = decryptor.update(binascii.unhexlify(ciphertext))
        actual_plaintext += decryptor.finalize()
...
def pbkdf2_test(backend, algorithm, params):
    ...
    # proper value.
    kdf = PBKDF2HMAC(algorithm, int(params['length']), params['salt'], int(params['iterations']), backend)
...
def aead_exception_test(backend, cipher_factory, mode_factory):
    cipher = Cipher(cipher_factory(binascii.unhexlify(b'0' * 32)), mode_factory(binascii.unhexlify(b'0' * 24)), backend)
    encryptor = cipher.encryptor()
    ...
    with pytest.raises(AlreadyFinalized):
        encryptor.finalize()
    cipher = Cipher(cipher_factory(binascii.unhexlify(b'0' * 32)), mode_factory(binascii.unhexlify(b'0' * 24), b'0' * 16), backend)
    decryptor = cipher.decryptor()
...
def aead_tag_exception_test(backend, cipher_factory, mode_factory):
    ...
    with pytest.raises(ValueError):
        Cipher(cipher_factory(binascii.unhexlify(b'0' * 32)), mode_factory(binascii.unhexlify(b'0' * 24), b'toolong' * 12), backend)
    with pytest.raises(ValueError):
...
def hkdf_expand_test(backend, algorithm, params):
    hkdf = HKDFExpand(algorithm, int(params['l']), info=binascii.unhexlify(params['info']) or None, backend=backend)
...
def _kbkdf_hmac_counter_mode_test(backend, prf, ctr_loc, params):
    supported_hash_algorithms: typing.Dict[str, typing.Type[hashes.HashAlgorithm]] = {'hmac_sha1': hashes.SHA1, 'hmac_sha224': hashes.SHA224, 'hmac_sha256': hashes.SHA256, 'hmac_sha384': hashes.SHA384, 'hmac_sha512': hashes.SHA512}
    algorithm = supported_hash_algorithms.get(prf)
    assert algorithm is not None
    assert backend.hmac_supported(algorithm())
    ctrkdf = KBKDFHMAC(algorithm(), Mode.CounterMode, params['l'] // 8, params['rlen'] // 8, None, ctr_loc, None, None, binascii.unhexlify(params['fixedinputdata']), backend=backend)
...
def _kbkdf_cmac_counter_mode_test(backend, prf, ctr_loc, params):
    supported_cipher_algorithms: typing.Dict[str, typing.Type[BlockCipherAlgorithm]] = {'cmac_aes128': algorithms.AES, 'cmac_aes192': algorithms.AES, 'cmac_aes256': algorithms.AES, 'cmac_tdes2': algorithms.TripleDES, 'cmac_tdes3': algorithms.TripleDES}
    algorithm = supported_cipher_algorithms.get(prf)
    assert algorithm is not None
    ctrkdf = KBKDFCMAC(algorithm, Mode.CounterMode, params['l'] // 8, params['rlen'] // 8, None, ctr_loc, None, None, binascii.unhexlify(params['fixedinputdata']), backend=backend)
    ko = ctrkdf.derive(binascii.unhexlify(params['ki']))
    assert binascii.hexlify(ko) == params['ko']

def kbkdf_counter_mode_test(backend, params):
    supported_counter_locations = {'before_fixed': CounterLocation.BeforeFixed, 'after_fixed': CounterLocation.AfterFixed}
    ...
    if ctr_loc is None or not isinstance(ctr_loc, CounterLocation):
        pytest.skip('Does not support counter location: {}'.format(params.get('ctrlocation')))
...
def generate_rsa_verification_test(param_loader, path, file_names, hash_alg, pad_factory):
    all_params = _load_all_params(path, file_names, param_loader)
...
def rsa_verification_test(backend, params, hash_alg, pad_factory):
    public_numbers = rsa.RSAPublicNumbers(e=params['public_exponent'], n=params['modulus'])
