...
class TestAESKeyWrap(object):
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.cipher_supported(algorithms.AES(b'\x00' * 16), modes.ECB()), skip_message='Does not support AES key wrap (RFC 3394) because AES-ECB is unsupported')
...
@pytest.mark.supported(only_if=lambda backend: backend.cipher_supported(algorithms.AES(b'\x00' * 16), modes.ECB()), skip_message='Does not support AES key wrap (RFC 5649) because AES-ECB is unsupported')
class TestAESKeyWrapWithPadding(object):

    def test_wrap(self, backend, subtests):
        params = _load_all_params(os.path.join('keywrap', 'kwtestvectors'), ['KWP_AE_128.txt', 'KWP_AE_192.txt', 'KWP_AE_256.txt'], load_nist_vectors)
        for param in params:
            with subtests.test():
                wrapping_key = binascii.unhexlify(param['k'])
                key_to_wrap = binascii.unhexlify(param['p'])
                wrapped_key = keywrap.aes_key_wrap_with_padding(wrapping_key, key_to_wrap, backend)
                assert param['c'] == binascii.hexlify(wrapped_key)
    ...
    def test_unwrap(self, backend, subtests):
        params = _load_all_params(os.path.join('keywrap', 'kwtestvectors'), ['KWP_AD_128.txt', 'KWP_AD_192.txt', 'KWP_AD_256.txt'], load_nist_vectors)
        for param in params:
            with subtests.test():
                wrapping_key = binascii.unhexlify(param['k'])
                wrapped_key = binascii.unhexlify(param['c'])
                if param.get('fail') is True:
                    with pytest.raises(keywrap.InvalidUnwrap):
                        keywrap.aes_key_unwrap_with_padding(wrapping_key, wrapped_key, backend)
                else:
                    unwrapped_key = keywrap.aes_key_unwrap_with_padding(wrapping_key, wrapped_key, backend)
                    assert param['p'] == binascii.hexlify(unwrapped_key)
    ...
    def test_unwrap_invalid_wrapped_key_length(self, backend):
        ...
        with pytest.raises(keywrap.InvalidUnwrap, match='Must be at least 16 bytes'):
            keywrap.aes_key_unwrap_with_padding(b'sixteen_byte_key', b'\x00' * 15, backend)
