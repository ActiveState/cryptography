...
class TestAESModeGCM(object):
    ...
    def test_buffer_protocol(self, backend):
        ...
        enc = base.Cipher(algorithms.AES(bytearray(b'\x00' * 16)), modes.GCM(bytearray(b'\x00' * 12)), backend).encryptor()
        enc.authenticate_additional_data(bytearray(b'foo'))
        ct = enc.update(data) + enc.finalize()
        dec = base.Cipher(algorithms.AES(bytearray(b'\x00' * 16)), modes.GCM(bytearray(b'\x00' * 12), enc.tag), backend).decryptor()
