...
class TestCMAC(object):
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.cmac_algorithm_supported(AES(fake_key)), skip_message='Does not support CMAC.')
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.cmac_algorithm_supported(TripleDES(fake_key)), skip_message='Does not support CMAC.')
    ...
    @pytest.mark.supported(only_if=lambda backend: backend.cipher_supported(ARC4(fake_key), None), skip_message='Does not support CMAC.')
    ...
    def test_buffer_protocol(self, backend):
        ...
        cmac = CMAC(AES(key), backend)
        cmac.update(b'6bc1bee22e409f96e93d7e117393172a')
        assert cmac.finalize() == binascii.unhexlify(b'a21e6e647bfeaf5ca0a5e1bcd957dfad')
