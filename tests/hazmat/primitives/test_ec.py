...
def _skip_ecdsa_vector(backend, curve_type, hash_type):
    if not backend.elliptic_curve_signature_algorithm_supported(ec.ECDSA(hash_type()), curve_type()):
        pytest.skip('ECDSA not supported with this hash {} and curve {}.'.format(hash_type().name, curve_type().name))

def _skip_curve_unsupported(backend, curve):
    if not backend.elliptic_curve_supported(curve):
        pytest.skip('Curve {} is not supported by this backend {}'.format(curve.name, backend))
...
def _skip_exchange_algorithm_unsupported(backend, algorithm, curve):
    if not backend.elliptic_curve_exchange_algorithm_supported(algorithm, curve):
        pytest.skip('Exchange with {} curve is not supported by {}'.format(curve.name, backend))
...
def test_ec_numbers():
    numbers = ec.EllipticCurvePrivateNumbers(1, ec.EllipticCurvePublicNumbers(2, 3, DummyCurve()))
...
@pytest.mark.parametrize(('private_value', 'x', 'y', 'curve'), [(None, 2, 3, DummyCurve()), (1, None, 3, DummyCurve()), (1, 2, None, DummyCurve()), (1, 2, 3, None)])
def test_invalid_ec_numbers_args(private_value, x, y, curve):
    with pytest.raises(TypeError):
        ec.EllipticCurvePrivateNumbers(private_value, ec.EllipticCurvePublicNumbers(x, y, curve))
...
def test_encode_point():
    ...
    assert data == binascii.unhexlify('04233ea3b0027127084cd2cd336a13aeef69c598d8af61369a36454a17c6c22aec3ea2c10a84153862be4ec82940f0543f9ba866af9751a6ee79d38460b35f442e')

def test_from_encoded_point():
    # secp256r1 point
    data = binascii.unhexlify('04233ea3b0027127084cd2cd336a13aeef69c598d8af61369a36454a17c6c22aec3ea2c10a84153862be4ec82940f0543f9ba866af9751a6ee79d38460b35f442e')
    with pytest.warns(CryptographyDeprecationWarning):
        pn = ec.EllipticCurvePublicNumbers.from_encoded_point(ec.SECP256R1(), data)
    assert pn.x == int('233ea3b0027127084cd2cd336a13aeef69c598d8af61369a36454a17c6c22aec', 16)
    assert pn.y == int('3ea2c10a84153862be4ec82940f0543f9ba866af9751a6ee79d38460b35f442e', 16)

def test_from_encoded_point_invalid_length():
    bad_data = binascii.unhexlify('04233ea3b0027127084cd2cd336a13aeef69c598d8af61369a36454a17c6c22aec3ea2c10a84153862be4ec82940f0543f9ba866af9751a6ee79d38460')
    with pytest.raises(ValueError):
        with pytest.warns(CryptographyDeprecationWarning):
...
def test_from_encoded_point_unsupported_point_no_backend():
    # set to point type 2.
    unsupported_type = binascii.unhexlify('02233ea3b0027127084cd2cd336a13aeef69c598d8af61369a36454a17c6c22a')
    with pytest.raises(ValueError):
        with pytest.warns(CryptographyDeprecationWarning):
            ec.EllipticCurvePublicNumbers.from_encoded_point(ec.SECP256R1(), unsupported_type)

def test_from_encoded_point_not_a_curve():
    with pytest.raises(TypeError):
        with pytest.warns(CryptographyDeprecationWarning):  # type: ignore[arg-type]
            ec.EllipticCurvePublicNumbers.from_encoded_point('notacurve', b'\x04data')
...
class TestECWithNumbers(object):
    ...
    def test_with_numbers(self, backend, subtests):
        vectors = itertools.product(load_vectors_from_file(os.path.join('asymmetric', 'ECDSA', 'FIPS_186-3', 'KeyPair.rsp'), load_fips_ecdsa_key_pair_vectors), _HASH_TYPES.values())
        for vector, hash_type in vectors:
            with subtests.test():
                curve_type: typing.Type[ec.EllipticCurve] = ec._CURVE_TYPES[vector['curve']]
                _skip_ecdsa_vector(backend, curve_type, hash_type)
                key = ec.EllipticCurvePrivateNumbers(vector['d'], ec.EllipticCurvePublicNumbers(vector['x'], vector['y'], curve_type())).private_key(backend)
                assert key
...
class TestECDSAVectors(object):
    ...
    def test_generate_unknown_curve(self, backend):
        with raises_unsupported_algorithm(exceptions._Reasons.UNSUPPORTED_ELLIPTIC_CURVE):
            ec.generate_private_key(DummyCurve(), backend)
        assert backend.elliptic_curve_signature_algorithm_supported(ec.ECDSA(hashes.SHA256()), DummyCurve()) is False
    ...
    def test_unknown_signature_algoritm(self, backend):
        ...
        with raises_unsupported_algorithm(exceptions._Reasons.UNSUPPORTED_PUBLIC_KEY_ALGORITHM), pytest.warns(CryptographyDeprecationWarning):
            key.signer(DummySignatureAlgorithm())
        with raises_unsupported_algorithm(exceptions._Reasons.UNSUPPORTED_PUBLIC_KEY_ALGORITHM):
        ...
        assert backend.elliptic_curve_signature_algorithm_supported(DummySignatureAlgorithm(), ec.SECP192R1()) is False

    def test_load_invalid_ec_key_from_numbers(self, backend):
        _skip_curve_unsupported(backend, ec.SECP256R1())
        numbers = ec.EllipticCurvePrivateNumbers(357646505660320080863666618182642070958081774038609089496899025506, ec.EllipticCurvePublicNumbers(47250808410327023131573602008345894927686381772325561185532964, 1120253292479243545483756778742719537373113335231773536789915, ec.SECP256R1()))
        with pytest.raises(ValueError):
            numbers.private_key(backend)
        numbers = ec.EllipticCurvePrivateNumbers(357646505660320080863666618182642070958081774038609089496899025506, ec.EllipticCurvePublicNumbers(-4725080841032702313157360200834589492768638177232556118553296, 1120253292479243545483756778742719537373113335231773536789915, ec.SECP256R1()))
        with pytest.raises(ValueError):
            numbers.private_key(backend)
        numbers = ec.EllipticCurvePrivateNumbers(357646505660320080863666618182642070958081774038609089496899025506, ec.EllipticCurvePublicNumbers(47250808410327023131573602008345894927686381772325561185532964, -1120253292479243545483756778742719537373113335231773536789915, ec.SECP256R1()))
        with pytest.raises(ValueError):
    ...
    def test_load_invalid_public_ec_key_from_numbers(self, backend):
        ...
        # Bad X coordinate
        numbers = ec.EllipticCurvePublicNumbers(int('000003647356b91f8ace114c7247ecf4f4a622553fc025e04a178f179ef279090c184af678a4c78f635483bdd8aa544851c6ef291c1f0d6a241ebfd14577d1d30d9903ce', 16), int('000001499bc7e079322ea0fcfbd6b40103fa6a1536c2257b182db0df4b3696ec643adf100eb4f2025d1b873f82e5a475d6e4400ba777090eeb4563a11509e4c87319dc26', 16), ec.SECP521R1())
        with pytest.raises(ValueError):
            numbers.public_key(backend)
        # Bad Y coordinate
        numbers = ec.EllipticCurvePublicNumbers(int('0000019aadc221cc0525118ab6d5aa1f64720603de0be128cbfea0b381ad802a2facc6370bb58cf88b3f0c692bc654ee19d6cad198f10d4b681b396f20d2e40603fa945b', 16), int('0000025da392803a320717a08d4cb3dea932039badff363b71bdb8064e7266c7f4f4b748d4d425347fc33e3885d34b750fa7fcd5691f4d90c89522ce33feff5db10088a5', 16), ec.SECP521R1())
        with pytest.raises(ValueError):
            numbers.public_key(backend)

    def test_signatures(self, backend, subtests):
        vectors = itertools.chain(load_vectors_from_file(os.path.join('asymmetric', 'ECDSA', 'FIPS_186-3', 'SigGen.txt'), load_fips_ecdsa_signing_vectors), load_vectors_from_file(os.path.join('asymmetric', 'ECDSA', 'SECP256K1', 'SigGen.txt'), load_fips_ecdsa_signing_vectors))
    ...
    def test_signature_failures(self, backend, subtests):
        vectors = load_vectors_from_file(os.path.join('asymmetric', 'ECDSA', 'FIPS_186-3', 'SigVer.rsp'), load_fips_ecdsa_signing_vectors)
        for vector in vectors:
            with subtests.test():
                hash_type = _HASH_TYPES[vector['digest_algorithm']]
                curve_type = ec._CURVE_TYPES[vector['curve']]
...
class TestECNumbersEquality(object):
    ...
    def test_private_numbers_eq(self):
        ...
        assert priv == ec.EllipticCurvePrivateNumbers(1, ec.EllipticCurvePublicNumbers(1, 2, ec.SECP192R1()))

    def test_private_numbers_ne(self):
        pub = ec.EllipticCurvePublicNumbers(1, 2, ec.SECP192R1())
        priv = ec.EllipticCurvePrivateNumbers(1, pub)
        assert priv != ec.EllipticCurvePrivateNumbers(2, ec.EllipticCurvePublicNumbers(1, 2, ec.SECP192R1()))
        assert priv != ec.EllipticCurvePrivateNumbers(1, ec.EllipticCurvePublicNumbers(2, 2, ec.SECP192R1()))
        assert priv != ec.EllipticCurvePrivateNumbers(1, ec.EllipticCurvePublicNumbers(1, 3, ec.SECP192R1()))
        assert priv != ec.EllipticCurvePrivateNumbers(1, ec.EllipticCurvePublicNumbers(1, 2, ec.SECP521R1()))
        assert priv != object()
...
class TestECSerialization(object):
    ...
    def test_private_bytes_encrypted_pem(self, backend, fmt, password):
        ...
        _skip_curve_unsupported(backend, ec.SECP256R1())
        key_bytes = load_vectors_from_file(os.path.join('asymmetric', 'PKCS8', 'ec_private_key.pem'), lambda pemfile: pemfile.read().encode())
    ...
    @pytest.mark.parametrize(('key_path', 'encoding', 'loader_func'), [[os.path.join('asymmetric', 'PEM_Serialization', 'ec_private_key.pem'), serialization.Encoding.PEM, serialization.load_pem_private_key], [os.path.join('asymmetric', 'DER_Serialization', 'ec_private_key.der'), serialization.Encoding.DER, serialization.load_der_private_key]])
    ...
    def test_private_bytes_traditional_der_encrypted_invalid(self, backend):
        _skip_curve_unsupported(backend, ec.SECP256R1())
        key = load_vectors_from_file(os.path.join('asymmetric', 'PKCS8', 'ec_private_key.pem'), lambda pemfile: serialization.load_pem_private_key(pemfile.read().encode(), None, backend))
        with pytest.raises(ValueError):
...
class TestEllipticCurvePEMPublicKeySerialization(object):
    ...
    @pytest.mark.parametrize(('key_path', 'loader_func', 'encoding'), [(os.path.join('asymmetric', 'PEM_Serialization', 'ec_public_key.pem'), serialization.load_pem_public_key, serialization.Encoding.PEM), (os.path.join('asymmetric', 'DER_Serialization', 'ec_public_key.der'), serialization.load_der_public_key, serialization.Encoding.DER)])
    ...
    def test_public_bytes_openssh(self, backend):
        _skip_curve_unsupported(backend, ec.SECP192R1())
        _skip_curve_unsupported(backend, ec.SECP256R1())
        key_bytes = load_vectors_from_file(os.path.join('asymmetric', 'PEM_Serialization', 'ec_public_key.pem'), lambda pemfile: pemfile.read(), mode='rb')
        ...
        assert ssh_bytes == b'ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBCS8827s9rUZyxZTi/um01+oIlWrwLHOjQxRU9CDAndom00zVAw5BRrIKtHB+SWD4P+sVJTARSq1mHt8kOIWrPc='
    ...
    @pytest.mark.parametrize('vector', load_vectors_from_file(os.path.join('asymmetric', 'EC', 'compressed_points.txt'), load_nist_vectors))
    def test_from_encoded_point_compressed(self, vector, backend):
        curve = {b'SECP256R1': ec.SECP256R1(), b'SECP256K1': ec.SECP256K1()}[vector['curve']]
        _skip_curve_unsupported(backend, curve)
    ...
    def test_from_encoded_point_notoncurve(self):
        uncompressed_point = binascii.unhexlify('047399336a9edf2197c2f8eb3d39aed9c34a66e45d918a07dc7684c42c9b37ac686699ececc4f5f0d756d3c450708a0694eb0a07a68b805070b40b058d27271f6e')
        with pytest.raises(ValueError):
            ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), uncompressed_point)

    def test_from_encoded_point_uncompressed(self):
        uncompressed_point = binascii.unhexlify('047399336a9edf2197c2f8eb3d39aed9c34a66e45d918a07dc7684c42c9b37ac686699ececc4f5f0d756d3c450708a0694eb0a07a68b805070b40b058d27271f6d')
        pn = ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), uncompressed_point)
        assert pn.public_numbers().x == int('7399336a9edf2197c2f8eb3d39aed9c34a66e45d918a07dc7684c42c9b37ac68', 16)
        assert pn.public_numbers().y == int('6699ececc4f5f0d756d3c450708a0694eb0a07a68b805070b40b058d27271f6d', 16)

    def test_from_encoded_point_invalid_length(self):
        bad_data = binascii.unhexlify('047399336a9edf2197c2f8eb3d39aed9c34a66e45d918a07dc7684c42c9b37ac686699ececc4f5f0d756d3c450708a0694eb0a07a68b805070b40b058d27271f6d')
        with pytest.raises(ValueError):
    ...
    def test_from_encoded_point_not_a_curve(self):
        with pytest.raises(TypeError):  # type: ignore[arg-type]
            ec.EllipticCurvePublicKey.from_encoded_point('notacurve', b'\x04data')

    def test_from_encoded_point_unsupported_encoding(self):
        unsupported_type = binascii.unhexlify('057399336a9edf2197c2f8eb3d39aed9c34a66e45d918a07dc7684c42c9b37ac68')
        with pytest.raises(ValueError):
            ec.EllipticCurvePublicKey.from_encoded_point(ec.SECP256R1(), unsupported_type)
    ...
    def test_serialize_point(self, vector, backend):
        ...
        key2 = ec.EllipticCurvePublicKey.from_encoded_point(curve, key.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint))
        assert key.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.CompressedPoint) == point
        assert key2.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.CompressedPoint) == point
...
class TestECDH(object):
    ...
    def test_key_exchange_with_vectors(self, backend, subtests):
        vectors = load_vectors_from_file(os.path.join('asymmetric', 'ECDH', 'KASValidityTest_ECCStaticUnified_NOKC_ZZOnly_init.fax'), load_kasvs_ecdh_vectors)
        for vector in vectors:
            with subtests.test():
                _skip_exchange_algorithm_unsupported(backend, ec.ECDH(), ec._CURVE_TYPES[vector['curve']]())
                key_numbers = vector['IUT']
                private_numbers = ec.EllipticCurvePrivateNumbers(key_numbers['d'], ec.EllipticCurvePublicNumbers(key_numbers['x'], key_numbers['y'], ec._CURVE_TYPES[vector['curve']]()))
                ...
                public_numbers = ec.EllipticCurvePublicNumbers(peer_numbers['x'], peer_numbers['y'], ec._CURVE_TYPES[vector['curve']]())
    ...
    @pytest.mark.parametrize('vector', load_vectors_from_file(os.path.join('asymmetric', 'ECDH', 'brainpool.txt'), load_nist_vectors))
    def test_brainpool_kex(self, backend, vector):
        ...
        key = ec.EllipticCurvePrivateNumbers(int(vector['da'], 16), ec.EllipticCurvePublicNumbers(int(vector['x_qa'], 16), int(vector['y_qa'], 16), curve)).private_key(backend)
        peer = ec.EllipticCurvePrivateNumbers(int(vector['db'], 16), ec.EllipticCurvePublicNumbers(int(vector['x_qb'], 16), int(vector['y_qb'], 16), curve)).private_key(backend)
    ...
    def test_exchange_unsupported_algorithm(self, backend):
        _skip_curve_unsupported(backend, ec.SECP256R1())
        key = load_vectors_from_file(os.path.join('asymmetric', 'PKCS8', 'ec_private_key.pem'), lambda pemfile: serialization.load_pem_private_key(pemfile.read().encode(), None, backend))
        with raises_unsupported_algorithm(exceptions._Reasons.UNSUPPORTED_EXCHANGE_ALGORITHM):
