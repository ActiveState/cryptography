...
class TestOpenSSL(object):
    ...
    def test_openssl_version_text(self):
        ...
        """
        assert backend.openssl_version_text().startswith('OpenSSL') or backend.openssl_version_text().startswith('LibreSSL')
    ...
    def test_nonexistent_cipher(self, mode):
        b = Backend()
        b.register_cipher_adapter(DummyCipherAlgorithm, type(mode), lambda backend, cipher, mode: backend._ffi.NULL)
    ...
    def test_unknown_error_in_cipher_finalize(self):
        cipher = Cipher(AES(b'\x00' * 16), CBC(b'\x00' * 16), backend=backend)
...
@pytest.mark.skipif(not backend._lib.CRYPTOGRAPHY_NEEDS_OSRANDOM_ENGINE, reason='Requires OpenSSL with ENGINE support and OpenSSL < 1.1.1d')
...
class TestOpenSSLRandomEngine(object):
    ...
    @pytest.mark.skipif(sys.executable is None, reason='No Python interpreter available.')
    def test_osrandom_engine_is_default(self, tmpdir):
        ...
        with engine_name.open('w') as out:
            subprocess.check_call([sys.executable, '-c', engine_printer], env=env, stdout=out, stderr=subprocess.PIPE)
        osrandom_engine_name = backend._ffi.string(backend._lib.Cryptography_osrandom_engine_name)
    ...
    def test_osrandom_engine_implementation(self):
        ...
        assert name in ['/dev/urandom', 'CryptGenRandom', 'getentropy', 'getrandom']
...
@pytest.mark.skipif(backend._lib.CRYPTOGRAPHY_NEEDS_OSRANDOM_ENGINE, reason='Requires OpenSSL without ENGINE support or OpenSSL >=1.1.1d')
class TestOpenSSLNoEngine(object):

    def test_no_engine_support(self):
        assert backend._ffi.string(backend._lib.Cryptography_osrandom_engine_id) == b'no-engine-support'
        assert backend._ffi.string(backend._lib.Cryptography_osrandom_engine_name) == b'osrandom_engine disabled'
...
class TestOpenSSLRSA(object):
    ...
    def test_rsa_padding_unsupported_pss_mgf1_hash(self):
        assert backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(DummyHashAlgorithm()), salt_length=0)) is False
    ...
    def test_rsa_padding_supported_pss(self):
        assert backend.rsa_padding_supported(padding.PSS(mgf=padding.MGF1(hashes.SHA1()), salt_length=0)) is True

    def test_rsa_padding_supported_oaep(self):
        assert backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA1(), label=None)) is True

    @pytest.mark.skipif(backend._lib.Cryptography_HAS_RSA_OAEP_MD == 0, reason='Requires OpenSSL with rsa_oaep_md (1.0.2+)')
    def test_rsa_padding_supported_oaep_sha2_combinations(self):
        ...
        for mgf1alg, oaepalg in itertools.product(hashalgs, hashalgs):
            assert backend.rsa_padding_supported(padding.OAEP(mgf=padding.MGF1(algorithm=mgf1alg), algorithm=oaepalg, label=None)) is True

    def test_rsa_padding_unsupported_mgf(self):  # type: ignore[arg-type]
        assert backend.rsa_padding_supported(padding.OAEP(mgf=DummyMGF(), algorithm=hashes.SHA1(), label=None)) is False
        assert backend.rsa_padding_supported(padding.PSS(mgf=DummyMGF(), salt_length=0)) is False

    @pytest.mark.skipif(backend._lib.Cryptography_HAS_RSA_OAEP_MD == 1, reason='Requires OpenSSL without rsa_oaep_md (< 1.0.2)')
    def test_unsupported_mgf1_hash_algorithm_decrypt(self):
        private_key = RSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.decrypt(b'0' * 64, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA1(), label=None))

    @pytest.mark.skipif(backend._lib.Cryptography_HAS_RSA_OAEP_MD == 1, reason='Requires OpenSSL without rsa_oaep_md (< 1.0.2)')
    def test_unsupported_oaep_hash_algorithm_decrypt(self):
        private_key = RSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.decrypt(b'0' * 64, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA1()), algorithm=hashes.SHA256(), label=None))

    def test_unsupported_mgf1_hash_algorithm_md5_decrypt(self):
        private_key = RSA_KEY_512.private_key(backend)
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_PADDING):
            private_key.decrypt(b'0' * 64, padding.OAEP(mgf=padding.MGF1(algorithm=hashes.MD5()), algorithm=hashes.MD5(), label=None))
...
class TestOpenSSLSignX509Certificate(object):
    ...
    def test_requires_certificate_builder(self):
        ...
        with pytest.raises(TypeError):  # type: ignore[arg-type]
            backend.create_x509_certificate(object(), private_key, DummyHashAlgorithm())

    def test_builder_requires_public_key(self):
        builder = x509.CertificateBuilder()
        private_key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(TypeError):
            backend.create_x509_certificate(builder, private_key, DummyHashAlgorithm())
...
class TestOpenSSLSignX509CSR(object):
    ...
    def test_requires_csr_builder(self):
        private_key = RSA_KEY_2048.private_key(backend)
        with pytest.raises(TypeError):  # type: ignore[arg-type]
            backend.create_x509_csr(object(), private_key, DummyHashAlgorithm())
...
class TestOpenSSLSerializationWithOpenSSL(object):
    ...
    def test_very_long_pem_serialization_password(self):
        password = b'x' * 1024
        with pytest.raises(ValueError):
            load_vectors_from_file(os.path.join('asymmetric', 'Traditional_OpenSSL_Serialization', 'key1.pem'), lambda pemfile: backend.load_pem_private_key(pemfile.read().encode(), password))
...
class TestRSAPEMSerialization(object):
    ...
    def test_password_length_limit(self):
        ...
        with pytest.raises(ValueError):
            key.private_bytes(serialization.Encoding.PEM, serialization.PrivateFormat.PKCS8, serialization.BestAvailableEncryption(password))

@pytest.mark.skipif(backend._lib.Cryptography_HAS_EVP_PKEY_DHX == 1, reason='Requires OpenSSL without EVP_PKEY_DHX (< 1.0.2)')
class TestOpenSSLDHSerialization(object):
    ...
    def test_dh_serialization_with_q_unsupported(self, backend, vector):
        ...
        with raises_unsupported_algorithm(_Reasons.UNSUPPORTED_SERIALIZATION):
            parameters.parameters(backend).parameter_bytes(serialization.Encoding.PEM, serialization.ParameterFormat.PKCS3)

    @pytest.mark.parametrize(('key_path', 'loader_func'), [(os.path.join('asymmetric', 'DH', 'dhkey_rfc5114_2.pem'), serialization.load_pem_private_key), (os.path.join('asymmetric', 'DH', 'dhkey_rfc5114_2.der'), serialization.load_der_private_key)])
    ...
    @pytest.mark.parametrize(('key_path', 'loader_func'), [(os.path.join('asymmetric', 'DH', 'dhpub_rfc5114_2.pem'), serialization.load_pem_public_key), (os.path.join('asymmetric', 'DH', 'dhpub_rfc5114_2.der'), serialization.load_der_public_key)])
...
def test_pyopenssl_csr_fallback():
    cert = _load_cert(os.path.join('x509', 'requests', 'rsa_sha256.pem'), x509.load_pem_x509_csr)
    req_ossl = None
    with pytest.warns(utils.CryptographyDeprecationWarning):
...
def test_pyopenssl_crl_fallback():
    cert = _load_cert(os.path.join('x509', 'PKITS_data', 'crls', 'GoodCACRL.crl'), x509.load_der_x509_crl)
    req_crl = None
    with pytest.warns(utils.CryptographyDeprecationWarning):
