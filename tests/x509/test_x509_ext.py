...
class TestCertificatePolicies(object):
    ...
    def test_long_oid(self, backend):
        ...
        """
        cert = _load_cert(os.path.join('x509', 'bigoid.pem'), x509.load_pem_x509_certificate, backend)
...
class TestCertificatePoliciesExtension(object):
    ...
    def test_cps_uri_policy_qualifier(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_cps_uri.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_user_notice_with_notice_reference(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_user_notice_with_notice_reference.pem'), x509.load_pem_x509_certificate, backend)
        cp = cert.extensions.get_extension_for_oid(ExtensionOID.CERTIFICATE_POLICIES).value
    ...
    def test_user_notice_with_explicit_text(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_user_notice_with_explicit_text.pem'), x509.load_pem_x509_certificate, backend)
        cp = cert.extensions.get_extension_for_oid(ExtensionOID.CERTIFICATE_POLICIES).value
    ...
    def test_user_notice_no_explicit_text(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_user_notice_no_explicit_text.pem'), x509.load_pem_x509_certificate, backend)
        cp = cert.extensions.get_extension_for_oid(ExtensionOID.CERTIFICATE_POLICIES).value
...
class TestExtensions(object):
    ...
    def test_one_extension(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'basic_constraints_not_critical.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_duplicate_extension(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'two_basic_constraints.pem'), x509.load_pem_x509_certificate, backend)
        with pytest.raises(x509.DuplicateExtension) as exc:
            cert.extensions
        assert exc.value.oid == ExtensionOID.BASIC_CONSTRAINTS

    def test_unsupported_critical_extension(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'unsupported_extension_critical.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(x509.ObjectIdentifier('1.2.3.4'))
        assert ext.value.value == b'value'

    def test_unsupported_extension(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'unsupported_extension_2.pem'), x509.load_pem_x509_certificate, backend)
        extensions = cert.extensions
...
class TestBasicConstraintsExtension(object):
    ...
    def test_ca_true_pathlen_6(self, backend):
        cert = _load_cert(os.path.join('x509', 'PKITS_data', 'certs', 'pathLenConstraint6CACert.crt'), x509.load_der_x509_certificate, backend)
    ...
    def test_path_length_zero(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'bc_path_length_zero.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.BASIC_CONSTRAINTS)
        assert ext is not None
        assert ext.critical is True
        assert ext.value.ca is True
    ...
    def test_no_basic_constraints(self, backend):
        cert = _load_cert(os.path.join('x509', 'PKITS_data', 'certs', 'ValidCertificatePathTest1EE.crt'), x509.load_der_x509_certificate, backend)
...
class TestKeyUsageExtension(object):
    ...
    def test_all_purposes(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'all_key_usages.pem'), x509.load_pem_x509_certificate, backend)
        extensions = cert.extensions
...
class TestRSAIssuerAlternativeNameExtension(object):
    ...
    def test_uri(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'ian_uri.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.ISSUER_ALTERNATIVE_NAME)
...
class TestRSASubjectAlternativeNameExtension(object):
    ...
    def test_wildcard_dns_name(self, backend):
        cert = _load_cert(os.path.join('x509', 'wildcard_san.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
    ...
    def test_san_empty_hostname(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_empty_hostname.pem'), x509.load_pem_x509_certificate, backend)
        san = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        dns = san.value.get_values_for_type(x509.DNSName)
        assert dns == ['']

    def test_san_wildcard_idna_dns_name(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_wildcard_idna.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        dns = ext.value.get_values_for_type(x509.DNSName)
        assert dns == ['*.xn--80ato2c.cryptography']

    def test_unsupported_gn(self, backend):
        cert = _load_cert(os.path.join('x509', 'san_x400address.der'), x509.load_der_x509_certificate, backend)
        with pytest.raises(x509.UnsupportedGeneralNameType):
            cert.extensions

    def test_registered_id(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_registered_id.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
        san = ext.value
    ...
    def test_uri(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_uri_with_port.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
    ...
    def test_ipaddress(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_ipaddr.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
        san = ext.value
    ...
    def test_dirname(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_dirname.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
        san = ext.value
    ...
    def test_rfc822name(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_rfc822_idna.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
        san = ext.value
    ...
    def test_idna2003_invalid(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_idna2003_dnsname.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_unicode_rfc822_name_dns_name_uri(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_idna_names.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
    ...
    def test_rfc822name_dnsname_ipaddress_directoryname_uri(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_email_dns_ip_dirname_uri.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
        san = ext.value
    ...
    def test_invalid_rfc822name(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_rfc822_names.pem'), x509.load_pem_x509_certificate, backend)
        san = cert.extensions.get_extension_for_class(x509.SubjectAlternativeName).value
    ...
    def test_other_name(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'san_other_name.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME)
        assert ext is not None
        assert ext.critical is False
...
class TestExtendedKeyUsageExtension(object):
    ...
    def test_eku(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'extended_key_usage.pem'), x509.load_pem_x509_certificate, backend)
...
class TestPolicyConstraintsExtension(object):
    ...
    def test_inhibit_policy_mapping(self, backend):
        cert = _load_cert(os.path.join('x509', 'department-of-state-root.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_require_explicit_policy(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'policy_constraints_explicit.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.POLICY_CONSTRAINTS)
        assert ext.critical is True
...
class TestSubjectInformationAccessExtension(object):
    ...
    def test_sia(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'sia.pem'), x509.load_pem_x509_certificate, backend)
...
class TestAuthorityInformationAccessExtension(object):
    ...
    def test_aia_multiple_ocsp_ca_issuers(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'aia_ocsp_ca_issuers.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_INFORMATION_ACCESS)
        assert ext is not None
        assert ext.critical is False
    ...
    def test_aia_ocsp_only(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'aia_ocsp.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_INFORMATION_ACCESS)
        assert ext is not None
        assert ext.critical is False
    ...
    def test_aia_ca_issuers_only(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'aia_ca_issuers.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_INFORMATION_ACCESS)
        assert ext is not None
        assert ext.critical is False
...
class TestAuthorityKeyIdentifierExtension(object):
    ...
    def test_aki_all_fields(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'authority_key_identifier.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_KEY_IDENTIFIER)
        assert ext is not None
        assert ext.critical is False
    ...
    def test_aki_no_keyid(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'authority_key_identifier_no_keyid.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_KEY_IDENTIFIER)
        assert ext is not None
        assert ext.critical is False
    ...
    def test_from_certificate(self, backend):
        issuer_cert = _load_cert(os.path.join('x509', 'rapidssl_sha256_ca_g3.pem'), x509.load_pem_x509_certificate, backend)
        cert = _load_cert(os.path.join('x509', 'cryptography.io.pem'), x509.load_pem_x509_certificate, backend)
        ext = cert.extensions.get_extension_for_oid(ExtensionOID.AUTHORITY_KEY_IDENTIFIER)
...
class TestNameConstraintsExtension(object):
    ...
    def test_permitted_excluded(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_permitted_excluded_2.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_permitted(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_permitted_2.pem'), x509.load_pem_x509_certificate, backend)
        nc = cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS).value
    ...
    def test_permitted_with_leading_period(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_permitted.pem'), x509.load_pem_x509_certificate, backend)
        nc = cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS).value
    ...
    def test_excluded_with_leading_period(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_excluded.pem'), x509.load_pem_x509_certificate, backend)
        nc = cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS).value
    ...
    def test_permitted_excluded_with_ips(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_permitted_excluded.pem'), x509.load_pem_x509_certificate, backend)
        nc = cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS).value
    ...
    def test_single_ip_netmask(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_single_ip_netmask.pem'), x509.load_pem_x509_certificate, backend)
        nc = cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS).value
    ...
    def test_ip_invalid_length(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_ip_invalid_length.pem'), x509.load_pem_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS)

    def test_invalid_ipv6_netmask(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_invalid_ip_netmask.pem'), x509.load_pem_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS)

    def test_invalid_ipv4_netmask(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'nc_invalid_ip4_netmask.der'), x509.load_der_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions.get_extension_for_oid(ExtensionOID.NAME_CONSTRAINTS)
    ...
    def test_certbuilder(self, backend):
        ...
        private_key = RSA_KEY_2048.private_key(backend)
        builder = _make_certbuilder(private_key)
        builder = builder.add_extension(NameConstraints(permitted_subtrees=list(map(DNSName, permitted)), excluded_subtrees=[]), True)
        cert = builder.sign(private_key, hashes.SHA256(), backend)
...
class TestCRLDistributionPointsExtension(object):
    ...
    def test_fullname_and_crl_issuer(self, backend):
        cert = _load_cert(os.path.join('x509', 'PKITS_data', 'certs', 'ValidcRLIssuerTest28EE.crt'), x509.load_der_x509_certificate, backend)
    ...
    def test_relativename_and_crl_issuer(self, backend):
        cert = _load_cert(os.path.join('x509', 'PKITS_data', 'certs', 'ValidcRLIssuerTest29EE.crt'), x509.load_der_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
    ...
    def test_fullname_crl_issuer_reasons(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cdp_fullname_reasons_crl_issuer.pem'), x509.load_pem_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
    ...
    def test_all_reasons(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cdp_all_reasons.pem'), x509.load_pem_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
    ...
    def test_single_reason(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cdp_reason_aa_compromise.pem'), x509.load_pem_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
    ...
    def test_crl_issuer_only(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cdp_crl_issuer.pem'), x509.load_pem_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
    ...
    def test_crl_empty_hostname(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'cdp_empty_hostname.pem'), x509.load_pem_x509_certificate, backend)
        cdps = cert.extensions.get_extension_for_oid(ExtensionOID.CRL_DISTRIBUTION_POINTS).value
...
class TestFreshestCRLExtension(object):
    ...
    def test_vector(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'freshestcrl.pem'), x509.load_pem_x509_certificate, backend)
...
class TestOCSPNoCheckExtension(object):
    ...
    def test_nocheck(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'ocsp_nocheck.pem'), x509.load_pem_x509_certificate, backend)
...
class TestInhibitAnyPolicyExtension(object):
    ...
    def test_inhibit_any_policy(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'inhibit_any_policy_5.pem'), x509.load_pem_x509_certificate, backend)
        iap = cert.extensions.get_extension_for_oid(ExtensionOID.INHIBIT_ANY_POLICY).value
...
class TestIssuingDistributionPointExtension(object):
    ...
    def test_vectors(self, filename, expected, backend):
        crl = _load_cert(os.path.join('x509', 'custom', filename), x509.load_pem_x509_crl, backend)
...
class TestPrecertPoisonExtension(object):
    ...
    def test_load(self, backend):
        cert = _load_cert(os.path.join('x509', 'cryptography.io.precert.pem'), x509.load_pem_x509_certificate, backend)
...
class TestSignedCertificateTimestamps(object):
    ...
    def test_eq(self, backend):
        sct = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        sct2 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        assert sct == sct2

    def test_ne(self, backend):
        sct = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        sct2 = _load_cert(os.path.join('x509', 'cryptography-scts.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        assert sct != sct2
        assert sct != object()
    ...
    def test_hash(self, backend):
        sct = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        sct2 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
        sct3 = _load_cert(os.path.join('x509', 'cryptography-scts.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value[0]
...
class TestPrecertificateSignedCertificateTimestampsExtension(object):
    ...
    def test_eq(self, backend):
        psct1 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        psct2 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        assert psct1 == psct2

    def test_ne(self, backend):
        psct1 = _load_cert(os.path.join('x509', 'cryptography-scts.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        psct2 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        assert psct1 != psct2
        assert psct1 != object()
    ...
    def test_hash(self, backend):
        psct1 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        psct2 = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        psct3 = _load_cert(os.path.join('x509', 'cryptography-scts.pem'), x509.load_pem_x509_certificate, backend).extensions.get_extension_for_class(x509.PrecertificateSignedCertificateTimestamps).value
        assert hash(psct1) == hash(psct2)
        assert hash(psct1) != hash(psct3)

    def test_simple(self, backend):
        cert = _load_cert(os.path.join('x509', 'badssl-sct.pem'), x509.load_pem_x509_certificate, backend)
    ...
    def test_invalid_version(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'invalid-sct-version.der'), x509.load_der_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions

    def test_invalid_length(self, backend):
        cert = _load_cert(os.path.join('x509', 'custom', 'invalid-sct-length.der'), x509.load_der_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions
...
class TestInvalidExtension(object):
    ...
    def test_invalid_certificate_policies_data(self, backend):
        ...
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_invalid.pem'), x509.load_pem_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions
        # CPSURI OID but UserNotice structure
        cert = _load_cert(os.path.join('x509', 'custom', 'cp_invalid2.der'), x509.load_der_x509_certificate, backend)
        with pytest.raises(ValueError):
            cert.extensions
