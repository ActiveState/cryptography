...
class TestCertificateRevocationListBuilder(object):
    ...
    def test_add_unsupported_extension(self, backend):
        ...
        last_update = datetime.datetime(2002, 1, 1, 12, 1)
        next_update = datetime.datetime(2030, 1, 1, 12, 1)
        builder = x509.CertificateRevocationListBuilder().issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, 'cryptography.io CA')])).last_update(last_update).next_update(next_update).add_extension(x509.OCSPNoCheck(), False)
    ...
    def test_sign_with_revoked_certificates(self, backend):
        ...
        builder = x509.CertificateRevocationListBuilder().issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, 'cryptography.io CA')])).last_update(last_update).next_update(next_update).add_revoked_certificate(revoked_cert0).add_revoked_certificate(revoked_cert1)
        crl = builder.sign(private_key, hashes.SHA256(), backend)
        assert len(crl) == 2
        assert crl.last_update == last_update
        assert crl.next_update == next_update
