...
def json_parametrize(keys, filename):
    ...
    with vector_file:
        ...
        return pytest.mark.parametrize(keys, [tuple([entry[k] for k in keys]) for entry in data])
...
class TestFernet(object):
    ...
    def test_generate(self, secret, now, iv, src, token, backend):
        ...
        actual_token = f._encrypt_from_parts(src.encode('ascii'), calendar.timegm(iso8601.parse_date(now).utctimetuple()), bytes(iv))
