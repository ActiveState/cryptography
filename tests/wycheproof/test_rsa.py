...
def test_rsa_pkcs1v15_signature_generation(backend, wycheproof):
    key = serialization.load_pem_private_key(wycheproof.testgroup['privateKeyPem'].encode(), password=None, backend=backend)
...
def test_rsa_oaep_encryption(backend, wycheproof):
    key = serialization.load_pem_private_key(wycheproof.testgroup['privateKeyPem'].encode('ascii'), password=None, backend=backend)
    assert isinstance(key, rsa.RSAPrivateKey)
    digest = _DIGESTS[wycheproof.testgroup['sha']]
    mgf_digest = _DIGESTS[wycheproof.testgroup['mgfSha']]
    assert digest is not None
    ...
    if not backend.rsa_padding_supported(padding_algo):
        pytest.skip('OAEP with digest={} and MGF digest={} not supported'.format(wycheproof.testgroup['sha'], wycheproof.testgroup['mgfSha']))
    if wycheproof.valid or wycheproof.acceptable:
