...
def test_aes_gcm(backend, wycheproof):
    ...
    if len(iv) < 8 or len(iv) > 128:
        pytest.skip('Less than 64-bit IVs (and greater than 1024-bit) are no longer supported')
    ...
    if wycheproof.valid or wycheproof.acceptable:
        ...
        dec = Cipher(algorithms.AES(key), modes.GCM(iv, tag, min_tag_length=len(tag)), backend).decryptor()
...
@pytest.mark.skipif(not _aead_supported(AESCCM), reason='Requires OpenSSL with AES-CCM support')
