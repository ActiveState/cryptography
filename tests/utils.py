...
def load_pkcs1_vectors(vector_data):
    ...
    vectors = []
    for line in vector_data:
        if line.startswith('# PSS Example') or line.startswith('# OAEP Example') or line.startswith('# PKCS#1 v1.5'):
            if example_vector:
...
def load_rsa_nist_vectors(vector_data):
    ...
    for line in vector_data:
        ...
        if name == 'n':
            ...
            if p is None:
                test_data = {'modulus': n, 'public_exponent': e, 'salt_length': salt_length, 'algorithm': value, 'fail': False}
            else:
...
def load_fips_dsa_key_pair_vectors(vector_data):
    ...
    for line in vector_data:
        ...
        if line.startswith('P'):
            ...
            vectors.append({'p': vectors[-1]['p'], 'q': vectors[-1]['q'], 'g': vectors[-1]['g'], 'x': int(line.split('=')[1], 16)})
...
def load_fips_dsa_sig_vectors(vector_data):
    ...
    """
    vectors = []
    sha_regex = re.compile('\\[mod = L=...., N=..., SHA-(?P<sha>1|224|256|384|512)\\]')
    for line in vector_data:
        line = line.strip()
        if not line or line.startswith('#'):
            continue
        ...
        if name == 'P':
            ...
            hexmsg = value.strip().encode('ascii')
            vectors.append({'p': vectors[-1]['p'], 'q': vectors[-1]['q'], 'g': vectors[-1]['g'], 'digest_algorithm': vectors[-1]['digest_algorithm'], 'msg': binascii.unhexlify(hexmsg)})
        elif name == 'X':
...
_ECDSA_CURVE_NAMES = {'P-192': 'secp192r1', 'P-224': 'secp224r1', 'P-256': 'secp256r1', 'P-384': 'secp384r1', 'P-521': 'secp521r1', 'K-163': 'sect163k1', 'K-233': 'sect233k1', 'K-256': 'secp256k1', 'K-283': 'sect283k1', 'K-409': 'sect409k1', 'K-571': 'sect571k1', 'B-163': 'sect163r2', 'B-233': 'sect233r1', 'B-283': 'sect283r1', 'B-409': 'sect409r1', 'B-571': 'sect571r1'}
...
def load_fips_ecdsa_signing_vectors(vector_data):
    ...
    """
    vectors = []
    curve_rx = re.compile('\\[(?P<curve>[PKB]-[0-9]{3}),SHA-(?P<sha>1|224|256|384|512)\\]')
    data = None
    for line in vector_data:
        line = line.strip()
        ...
        if curve_match:
            ...
            data = {'curve': curve_name, 'digest_algorithm': digest_name, 'message': binascii.unhexlify(hexmsg)}
...
def load_kasvs_ecdh_vectors(vector_data):
    ...
    """
    curve_name_map = {'P-192': 'secp192r1', 'P-224': 'secp224r1', 'P-256': 'secp256r1', 'P-384': 'secp384r1', 'P-521': 'secp521r1'}
    result_rx = re.compile('([FP]) \\(([0-9]+) -')
    tags = []
    sets = {}
    vectors = []
    ...
    for line in vector_data:
        ...
        if len(tags) == len(sets):
            break
    # Data
    data: typing.Dict[str, typing.Any] = {'CAVS': {}, 'IUT': {}}
    tag = None
    for line in vector_data:
        line = line.strip()
        if not line or line.startswith('#'):
...
class WycheproofTest(object):
    ...
    def __repr__(self):
        return '<WycheproofTest({!r}, {!r}, {!r}, tcId={})>'.format(self.testfiledata, self.testgroup, self.testcase, self.testcase['tcId'])
