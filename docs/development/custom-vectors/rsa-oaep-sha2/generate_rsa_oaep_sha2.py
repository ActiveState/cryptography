...
def build_vectors(mgf1alg, hashalg, filename):
    ...
    for vector in vectors:
        ...
        vectorkey = rsa.RSAPrivateNumbers(p=private['p'], q=private['q'], d=private['private_exponent'], dmp1=private['dmp1'], dmq1=private['dmq1'], iqmp=private['iqmp'], public_numbers=rsa.RSAPublicNumbers(e=private['public_exponent'], n=private['modulus'])).private_key(backend)
        count = 1
        for example in examples:
            ...
            output.append('# OAEP Example {0} alg={1} mgf1={2}'.format(count, hashalg.name, mgf1alg.name))
            count += 1
...
for hashtuple in itertools.product(hashalgs, hashalgs):
    if isinstance(hashtuple[0], hashes.SHA1) and isinstance(hashtuple[1], hashes.SHA1):
        continue
    write_file(build_vectors(hashtuple[0], hashtuple[1], oaep_path), 'oaep-{0}-{1}.txt'.format(hashtuple[0].name, hashtuple[1].name))