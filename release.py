...
def wait_for_build_complete_github_actions(session, token, run_url):
    while True:
        response = session.get(run_url, headers={'Content-Type': 'application/json', 'Authorization': 'token {}'.format(token)})
...
def download_artifacts_github_actions(session, token, run_url):
    response = session.get(run_url, headers={'Content-Type': 'application/json', 'Authorization': 'token {}'.format(token)})
    response.raise_for_status()
    response = session.get(response.json()['artifacts_url'], headers={'Content-Type': 'application/json', 'Authorization': 'token {}'.format(token)})
    response.raise_for_status()
    paths = []
    for artifact in response.json()['artifacts']:
        response = session.get(artifact['archive_download_url'], headers={'Content-Type': 'application/json', 'Authorization': 'token {}'.format(token)})
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            for name in z.namelist():
                if not name.endswith('.whl'):
                    continue
                p = z.open(name)
                out_path = os.path.join(os.path.dirname(__file__), 'dist', os.path.basename(name))
...
def fetch_github_actions_wheels(token, version):
    ...
    response = session.get('https://api.github.com/repos/pyca/cryptography/actions/workflows/wheel-builder.yml/runs?event=create', headers={'Content-Type': 'application/json', 'Authorization': 'token {}'.format(token)})
    response.raise_for_status()
