...
def build_ffi_for_binding(module_name, module_prefix, modules, libraries=[], extra_compile_args=[], extra_link_args=[]):
    ...
    * ``CUSTOMIZATIONS``: A string containing arbitrary top-level C code, this
    ...
    ffi = build_ffi(module_name, cdef_source='\n'.join(types + functions), verify_source=verify_source, libraries=libraries, extra_compile_args=extra_compile_args, extra_link_args=extra_link_args)
    return ffi

def build_ffi(module_name, cdef_source, verify_source, libraries=[], extra_compile_args=[], extra_link_args=[]):
    ffi = FFI()
    # Always add the CRYPTOGRAPHY_PACKAGE_VERSION to the shared object
    cdef_source += '\nstatic const char *const CRYPTOGRAPHY_PACKAGE_VERSION;'
    verify_source += '\n#define CRYPTOGRAPHY_PACKAGE_VERSION "{}"'.format(about['__version__'])
    ffi.cdef(cdef_source)
    ffi.set_source(module_name, verify_source, libraries=libraries, extra_compile_args=extra_compile_args, extra_link_args=extra_link_args)
    return ffi
