...
def _extra_compile_args(platform):
    ...
    if get_default_compiler() == 'unix':
        ...
        cmd = config(d)
        cmd._check_compiler()
        is_gcc = 'gcc' in cmd.compiler.compiler[0] or 'clang' in cmd.compiler.compiler[0]
    if is_gcc or not (platform in ['win32', 'hp-ux11', 'sunos5'] or platform.startswith('aix')):
        return ['-Wconversion', '-Wno-error=sign-conversion']
    else:
        return []
# This goes first so we can define some cryptography-wide symbols.
# Provider comes early as well so we define OSSL_LIB_CTX
ffi = build_ffi_for_binding(module_name='_openssl', module_prefix='_cffi_src.openssl.', modules=['cryptography', 'provider', 'aes', 'asn1', 'bignum', 'bio', 'cmac', 'conf', 'crypto', 'dh', 'dsa', 'ec', 'ecdh', 'ecdsa', 'engine', 'err', 'evp', 'fips', 'hmac', 'nid', 'objects', 'ocsp', 'opensslv', 'osrandom_engine', 'pem', 'pkcs12', 'rand', 'rsa', 'ssl', 'x509', 'x509name', 'x509v3', 'x509_vfy', 'pkcs7', 'callbacks'], libraries=_get_openssl_libraries(sys.platform), extra_compile_args=_extra_compile_args(sys.platform), extra_link_args=extra_link_args(compiler_type()))