...
class Backend(BackendInterface):
    ...
    def hash_supported(self, algorithm):
        if self._fips_enabled and (
            not isinstance(algorithm, self._fips_hashes)
        ):
            return False
        evp_md = self._evp_md_from_algorithm(algorithm)
    ...
    def _x509_check_signature_params(self, private_key, algorithm):
        ...
        elif isinstance(algorithm, hashes.MD5) and (
    ...
    def _crl_is_signature_valid(self, crl, public_key):
        if not isinstance(
            ...
            raise TypeError(
                "Expecting one of DSAPublicKey, RSAPublicKey, or EllipticCurvePublicKey."
            )
    ...
    def _load_key(self, openssl_read_func, convert_func, data, password):
        ...
        if evp_pkey == self._ffi.NULL:
            if userdata.error != 0:
                ...
                if userdata.error == -1:
                    ...
                    raise ValueError(
                        "Passwords longer than {} bytes are not supported by this backend.".format(
                            userdata.maxsize - 1
                        )
                    )
            else:
        ...
        assert (
            password is not None and userdata.called == 1 or password is None
        )
    ...
    def elliptic_curve_exchange_algorithm_supported(self, algorithm, curve):
        if self._fips_enabled and (
            not isinstance(curve, self._fips_ecdh_curves)
        ):
            return False
    ...
    def derive_scrypt(self, key_material, salt, length, n, r, p):
        ...
        if res != 1:
            ...
            raise MemoryError(
                "Not enough memory to derive key. These parameters require {} MB of memory.".format(
                    min_memory
                ),
                errors,
            )
    ...
    def _load_pkcs7_certificates(self, p7):
        ...
        if nid != self._lib.NID_pkcs7_signed:
            raise UnsupportedAlgorithm(
                "Only basic signed structures are currently supported. NID for this data was {}".format(
                    nid
                ),
