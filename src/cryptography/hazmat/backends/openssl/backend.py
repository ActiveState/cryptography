...
class Backend(BackendInterface):
    ...
    _fips_aead = {b'aes-128-ccm', b'aes-192-ccm', b'aes-256-ccm', b'aes-128-gcm', b'aes-192-gcm', b'aes-256-gcm'}
    ...
    def _register_default_ciphers(self):
        ...
        for cipher_cls, mode_cls in itertools.product([CAST5, IDEA], [CBC, OFB, CFB, ECB]):
    ...
    def _x509_check_signature_params(self, private_key, algorithm):
        ...
        elif isinstance(algorithm, hashes.MD5) and (not isinstance(private_key, rsa.RSAPrivateKey)):
    ...
    def create_x509_csr(self, builder, private_key, algorithm):
        ...
        self._create_x509_extensions(extensions=builder._extensions, handlers=self._extension_encode_handlers, x509_obj=sk_extension, add_func=self._lib.sk_X509_EXTENSION_insert, gc=False)
    ...
    def create_x509_certificate(self, builder, private_key, algorithm):
        ...
        # Add extensions.
        self._create_x509_extensions(extensions=builder._extensions, handlers=self._extension_encode_handlers, x509_obj=x509_cert, add_func=self._lib.X509_add_ext, gc=True)
        # Set the issuer name.
    ...
    def create_x509_crl(self, builder, private_key, algorithm):
        ...
        self.openssl_assert(res == 1)
        # Add extensions.
        self._create_x509_extensions(extensions=builder._extensions, handlers=self._crl_extension_encode_handlers, x509_obj=x509_crl, add_func=self._lib.X509_CRL_add_ext, gc=True)
        ...
        for revoked_cert in builder._revoked_certificates:
            ...
            self._create_x509_extensions(extensions=revoked_cert.extensions, handlers=self._crl_entry_extension_encode_handlers, x509_obj=x509_revoked, add_func=self._lib.X509_REVOKED_add_ext, gc=True)
            res = self._lib.X509_CRL_add0_revoked(x509_crl, x509_revoked)
            self.openssl_assert(res == 1)  # type: ignore[union-attr]
    ...
    def _create_x509_extension(self, handlers, extension):
        if isinstance(extension.value, x509.UnrecognizedExtension):
            ...
            return self._create_raw_x509_extension(extension, value)
        elif isinstance(extension.value, x509.PrecertificateSignedCertificateTimestamps):
    ...
    def _ossl2crl(self, x509_crl):
        bio = self._create_mem_bio_gc()
        res = self._lib.i2d_X509_CRL_bio(bio, x509_crl)
        self.openssl_assert(res == 1)
        return rust_x509.load_der_x509_crl(self._read_mem_bio(bio))

    def _crl_is_signature_valid(self, crl, public_key):
        if not isinstance(public_key, (_DSAPublicKey, _RSAPublicKey, _EllipticCurvePublicKey)):
    ...
    def _csr_is_signature_valid(self, csr):
    ...
    def create_ocsp_request(self, builder):
        ...
        self._create_x509_extensions(extensions=builder._extensions, handlers=self._ocsp_request_extension_encode_handlers, x509_obj=ocsp_req, add_func=self._lib.OCSP_REQUEST_add_ext, gc=True)
        bio = self._create_mem_bio_gc()
    ...
    def _create_ocsp_basic_response(self, builder, private_key, algorithm):
        ...
        if builder._certs is not None:
            for cert in builder._certs:
                ...
                self.openssl_assert(res == 1)
        self._create_x509_extensions(extensions=builder._extensions, handlers=self._ocsp_basicresp_extension_encode_handlers, x509_obj=basic, add_func=self._lib.OCSP_BASICRESP_add_ext, gc=True)
    ...
    def load_dh_private_numbers(self, numbers):
        ...
        if codes[0] != 0 and (not (parameter_numbers.g == 2 and codes[0] ^ self._lib.DH_NOT_SUITABLE_GENERATOR == 0)):
