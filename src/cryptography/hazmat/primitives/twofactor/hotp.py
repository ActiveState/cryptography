...
def _generate_uri(hotp, type_name, account_name, issuer, extra_parameters):
    ...
    uriparts = {'type': type_name, 'label': '%s:%s' % (quote(issuer), quote(account_name)) if issuer else quote(account_name), 'parameters': urlencode(parameters)}
    return 'otpauth://{type}/{label}?{parameters}'.format(**uriparts)

class HOTP(object):

    def __init__(self, key, length, algorithm, backend=None, enforce_key_length=True):
