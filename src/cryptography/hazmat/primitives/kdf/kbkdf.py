...
class _KBKDFDeriver:
    ...
    def __init__(self, prf, mode, length, rlen, llen, location, label, context, fixed):
        assert callable(prf)
    ...
    def derive(self, key_material, prf_output_size):
        if self._used:
...
class KBKDFHMAC(KeyDerivationFunction):
    ...
    def __init__(self, algorithm, mode, length, rlen, llen, location, label, context, fixed, backend=None):
        ...
        self._deriver = _KBKDFDeriver(self._prf, mode, length, rlen, llen, location, label, context, fixed)

    def _prf(self, key_material):
